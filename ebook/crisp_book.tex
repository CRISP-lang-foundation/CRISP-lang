...
\texttt{exit} & \texttt{exit(code?) → never returns} &
Terminates process with exit code. \texttt{finally} blocks execute. Not catchable.\\
\texttt{\_exit} & \texttt{\_exit(code?) → never returns} &
Hard process exit. No cleanup, skips \texttt{finally}.\\
