...
# Storing methods in hashes
let %obj = {
    name => "CRISP",
    greet => fn() { return "Hello!"; }
};
say %obj.greet();  # Hello!
\end{lstlisting}
\end{tcolorbox}
...
\texttt{Array methods:} \texttt{.push(val)}, \texttt{.pop()}, \texttt{.shift()}, \texttt{.unshift(val)}, \texttt{.len()}, \texttt{.map(callback)}, \texttt{.filter(callback)}, \texttt{.grep(callback)}, \texttt{.join(sep)}

\texttt{String methods:} \texttt{.len()}, \texttt{.split(delim)}
...
& & \texttt{catch e \{ say e; \}} \\
& & \texttt{catch e as err \{ say err; \}} (alias syntax) \\
...
& & \texttt{print("Hello, ", name);} (paren syntax) \\

\texttt{say} & Output &
Outputs arguments to stdout with automatic trailing newline.\\
& & \texttt{say "Hello, ", name, "!";} \\
& & \texttt{say("Hello, ", name);} (paren syntax) \\
