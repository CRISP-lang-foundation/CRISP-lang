...
\begin{tcolorbox}[colback=darkpurple, colframe=darkpurple, width=4cm, center, sharp corners, boxrule=0pt]
    \centering
    {\large\bfseries\color{codegold}Version 0.1.0}
\end{tcolorbox}
...
let fd = posix["open"]("data.txt", O_WRONLY | O_CREAT, 0o644);
posix["write"](fd, "Hello POSIX!\n", 13);
posix["close"](fd);

# Seek in file
let fd2 = posix["open"]("data.bin", O_RDONLY);
posix["lseek"](fd2, 0, SEEK_END);
let size = posix["lseek"](fd2, 0, SEEK_CUR);
say "File size: ", size;
posix["close"](fd2);
\end{lstlisting}
\end{tcolorbox}
